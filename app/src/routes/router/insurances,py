...
async def get_insurances(db_session: Session = Depends(get_db_connect), token: str = Depends(oauth2_scheme)):
    """
    ## RESPONSE
...
async def create_insurance(insurance: InsurancesRequest, db_session: Session = Depends(get_db_connect), token: str = Depends(oauth2_scheme)):
    """
    ## REQUEST BODY
        - insurance_id: int
        - name: str
...
@router.put(path="/insurances{insurance_id}", status_code=status.HTTP_200_OK, summary="Update Insurance")
async def update_insurance(insurance_id: int, insurance: InsurancesUpdateRequest, db_session: Session = Depends(get_db_connect), token: str = Depends(oauth2_scheme)):
    """
    ## REQUEST BODY
        - name: str
        - description: str
...
@router.delete(path="/insurances{insurance_id}", status_code=status.HTTP_200_OK, summary="Delete Insurance")
async def delete_insurance(insurance_id: int, db_session: Session = Depends(get_db_connect), token: str = Depends(oauth2_scheme)):
    """
    ## RESPONSE
