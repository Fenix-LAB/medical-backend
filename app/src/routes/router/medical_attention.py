...
@router.get(path="/medical-attentions", status_code=status.HTTP_200_OK, summary="Get All Medical Attentions")
async def get_medical_attentions(db_session: Session = Depends(get_db_connect), token: str = Depends(oauth2_scheme)):
    """
    ## RESPONSE
...
@router.post(path="/medical-attentions", status_code=status.HTTP_201_CREATED, summary="Create Medical Attention")
async def create_medical_attention(medical_attention: MedicalAttentionRequest, db_session: Session = Depends(get_db_connect), token: str = Depends(oauth2_scheme)):
    """
    ## REQUEST BODY
...
@router.put(path="/medical-attentions/{attention_id}", status_code=status.HTTP_200_OK, summary="Update Medical Attention")
async def update_medical_attention(medical_attention: MedicalAttentionUpdateRequest, attention_id: int,  db_session: Session = Depends(get_db_connect), token: str = Depends(oauth2_scheme)):
    """
    ## REQUEST BODY
        - appointment_id: int (optional)
        - establishment_id: int (optional)
...
@router.delete(path="/medical-attentions/{attention_id}", status_code=status.HTTP_200_OK, summary="Delete Medical Attention")
async def delete_medical_attention(attention_id: int, db_session: Session = Depends(get_db_connect), token: str = Depends(oauth2_scheme)):
    """
    ## RESPONSE
