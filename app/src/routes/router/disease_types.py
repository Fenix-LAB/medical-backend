...
@router.post(
    path="/disease_types", status_code=status.HTTP_201_CREATED, summary="Create Disease Type"
)
async def create_disease_type(
    disease_type: DiseaseTypesRequest,
    ...
    result = diseases_types.create(disease_type, db_session, payload)
    if isinstance(result, Exception):
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(result))
...
@router.put(
    path="/disease_types{disease_type_id}",
    status_code=status.HTTP_200_OK,
    summary="Update Disease Type",
)
async def update_disease_type(
    disease_type: DiseaseTypesUpdateRequest,
    disease_type_id: int,
    db_session: Session = Depends(get_db_connect),
    token: str = Depends(oauth2_scheme),
):
    ...
    result = diseases_types.update(disease_type, disease_type_id, db_session, payload)
    if isinstance(result, Exception):
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=str(result))
