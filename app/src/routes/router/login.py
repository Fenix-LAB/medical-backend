...
def new_token(old_token: str = None, session_id: str = None,
    ...
    if old_token and session_id:
        ...
        if email == db_session.email and (
            ...
            crud_login.active_user(session_id=session_id, db=db)
            access_token_expires = timedelta(
                minutes=ProjectSettings.ACCESS_TOKEN_EXPIRE_MINUTES)
            token = access_token.create_access_token(
                data={"sub": email},
...
def logoff_user(session_id: str,
                db: Session = Depends(get_db_connect)) -> JSONResponse:
    """ Login user and Return Access Token"""
    db_session = crud_login.logoff_user(session_id=session_id, db=db)
...
@router.post("/password_recovery",
             responses=response_schemas.general_responses,
             include_in_schema=False)
def recover_password(user: schemas.UserBase,
                     db: Session = Depends(get_db_connect)) -> JSONResponse:
    """
    Password Recovery
    """
...
def reset_password(reset_data: schemas.UserPasswordReset,
    ...
    data = crud_users.update_user_password(email=token_email,
                                           password=reset_data.password, db=db)
    if data is None:
        return JSONResponse(status_code=500,
                            content={"message": "Internal Server Error"})
    return JSONResponse(status_code=200,
