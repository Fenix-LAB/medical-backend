...
async def get_exam_types(db_session: Session = Depends(get_db_connect)):
    """
    ## RESPONSE
...
async def create_exam_type(exam_type: ExamTypesRequest, db_session: Session = Depends(get_db_connect)):
    """
    ## REQUEST BODY
        - company_id: int
        - exam_name: str
...
async def update_exam_type(exam_type: ExamTypesUpdateRequest, exam_type_id: int, db_session: Session = Depends(get_db_connect)):
    """
    ## REQUEST BODY
...
async def delete_exam_type(exam_type_id: int, db_session: Session = Depends(get_db_connect)):
    """
    ## REQUEST PARAMS
        - exam_type_id: int
