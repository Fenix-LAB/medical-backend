...
def create(person: PersonsRequest, db_session: Session, payload):
    ...
    try:
        ...
        return {"message": "Person created successfully", "data": person.dict()}
...
def update(person: PersonsUpdateRequest, person_id: int, db_session: Session, payload):
    ...
    try:
        ...
        query = text(
            "UPDATE persons SET first_name = :first_name, last_name = :last_name, identification_type = :identification_type, identification = :identification, birthdate = :birthdate, gender = :gender, marital_status = :marital_status, address = :address, phone_number = :phone_number, email = :email, updated_at = :updated_at, updated_by = :updated_by, company_id = :company_id WHERE person_id = :person_id"
        )
